...
def show
  ...
  def like
    recruiter_card = RecruiterCard.find(params[:id])
    new_like = RecruiterCardLike.create({ :user_id => current_user.id, :card_id => recruiter_card.id, :card_name => recruiter_card.name, :card_details => recruiter_card.description })
